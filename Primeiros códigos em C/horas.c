...
int main(void)
{
    ...
    {
        horas[i] = get_int("Horas na semana %i: ", i + 1);
    }
...
float calcular_horas(int horas[], int semanas, char saida)
{
    ...
    // Calcula o total de horas de estudo ou a média de horas por semana
    for (int i = 0; i < semanas; i++)
    {
        total_horas += horas[i];
    }
    ...
    if (saida == 'M')
    {
        return total_horas / semanas;
    }
    else // Saída padrão é o total de horas
    {
        return total_horas;
    }
}
